...
- **FR-13.4 (Super Admin Dashboard):** Dedicated dashboard showing system overview KPIs (user accounts from `GET /users/stats`, active accounts — lifecycle status `ACTIVE` per FR-13.6, sub-departments, API health), seed/migration status from `GET /system-metadata`, deactivated accounts requiring attention, recently provisioned accounts, role distribution, system health status, and system activity feed.
...
- **FR-13.9 (Leadership Handover Workflow):** A guided three-step sequence for the annual leadership rotation: create successor account → demote the outgoing leader off the leadership role (`PATCH /users/:id` to a non-leadership role) → deactivate the outgoing leader's account, with BR-009 conflict validation at each step.
