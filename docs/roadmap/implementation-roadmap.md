...
### Phase 6.1: Super Admin Planned Enhancements (v2.2 — ✅ Delivered)

> From the September 2026 Super Admin review session. Requirement text: FR-13.6 – FR-13.13; full descriptions: system documentation § 7.10.6. All eight enhancements are implemented and tested.

- **Modules:** `apps/admin/features/users`, `apps/admin/features/audit-logs`, `apps/admin/features/dashboard`.
- **API:** `POST /users/:id/reactivate`, `POST /users/:id/revoke-sessions`; audit filters/export on `/api/v1/audit-logs`; guard rails in the users use-cases.
- **Delivered:**
  1. ✅ **Account reactivation** (FR-13.6) — `POST /users/:id/reactivate`, audit action `USER_REACTIVATED`, one-click from the deactivated-accounts widget.
  2. ✅ **Edit user / role reassignment** (FR-13.7) — edit dialog on `PATCH /users/:id` with BR-007/BR-009 revalidation and field-level audit diffs.
