          ...
          <>
            ...
            <dl className="mt-3">
              {[
                ...
                ...(schemaTag ? [{ term: "Schema", detail: schemaTag }] : []),
                ...(seedStatus ? [{ term: "Seed", detail: seedStatus }] : []),
              ].map((row) => (
                <div
                  key={row.term}
...
export function SuperAdminDashboardPage() {
  ...
  const schemaTag = systemMetadata.find((row) => row.key === "schema_tag")?.value ?? null;
  const seedStatus = systemMetadata.find((row) => row.key === "seed_status")?.value ?? null;
      ...
      <div className="w-full">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,3.4fr)_minmax(260px,0.9fr)] lg:gap-x-[18px] lg:gap-y-4">
          ...
          <SystemSnapshot
            ...
            deactivatedCount={userStats?.deactivated ?? deactivatedAccounts.length}
