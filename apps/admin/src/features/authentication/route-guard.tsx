...
import { hasPortalAccess, isLeadershipRole } from "@/authorization";
    ...
    const checkAuth = async () => {
      try {
        ...
        let sessionUser: SessionUser = {
          ...
          name: metadataName,
          role: metadataRole,
          globalRoles: isLeadershipRole(metadataRole) ? [metadataRole] : [],
          subDeptRoles: [],
        };
        ...
        if (isMounted) {
          ...
          // ADR-0007: Non-leader lockout check — centralized in authorization.
          setAuthorized(hasPortalAccess(sessionUser.globalRoles, sessionUser.subDeptRoles));
        }
      } catch {
        if (isMounted) {
          router.push(`/login?redirect=${encodeURIComponent(pathname)}`);
