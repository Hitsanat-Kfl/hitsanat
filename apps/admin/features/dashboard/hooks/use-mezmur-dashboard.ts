...
export function useMezmurDashboard(): UseMezmurDashboardResult {
  ...
  const [attentionItems, setAttentionItems] = useState<ListItemData[]>([]);
  const [distributionSummary, setDistributionSummary] = useState<StatusSummaryItem[]>([]);
  const [sessionSummary, setSessionSummary] = useState<StatusSummaryItem[]>([]);
  const [responsibilities, setResponsibilities] = useState<ListItemData[]>([]);
  const [upcomingPrograms, setUpcomingPrograms] = useState<EventItem[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    if (departmentsRes.status === "rejected") {
      ...
      setError(reason instanceof Error ? reason.message : "Failed to load Mezmur department data");
      setLoading(false);
      return;
    }
    ...
    setAttentionItems(attentionQueue);
    ...
    setDistributionSummary([
      ...
      { status: "completed", label: "Completed", count: distCompleted },
    ]);
