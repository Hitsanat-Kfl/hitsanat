...
const STEPS = ["Provision the successor's account", "Deactivate the outgoing leader"] as const;

/**
 * PE-04 / FR-13.9: guided leadership handover for the annual rotation.
 * Walks the documented sequence with validation at each step:
 *   1. create the successor's account (BR-008) linked to the incoming
 *      member (BR-007), taking over the outgoing leader's role;
 *   2. deactivate the outgoing leader's account.
 */
    ...
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Guided rotation: provision the successor with the same role, then deactivate {user.name}
            &apos;s account. BR-009 conflicts are validated at each step.
          </DialogDescription>
        </DialogHeader>
        ...
        {done ? (
          ...
          <div className="space-y-4">
            <p className="text-sm">
              The successor account is provisioned. Deactivating{" "}
